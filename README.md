...
`atlas init-db` is optional for first run: `PipelineOrchestrator.initialize()` calls `create_tables()`. Use `--force` only when you intend to drop and recreate tables.
...
Non-dry-run backfills print an estimate and require interactive confirmation (`Proceed with backfill?`). `BackfillConfig` defaults to `skip_weekends=True` and `skip_holidays=False` (no holiday calendar is implemented). `resume_from` and `parallel_batches` exist on the dataclass but are not exposed by the CLI. The printed estimate assumes **5 seconds per date** and ignores ticker count.

**Fetch strategy (verified):** `atlas backfill` is **not** a range fetch. It runs the orchestrator once per weekday, and each provider call uses `fetch_data(target_date)` (same-day `startDate`/`endDate` or FRED `observation_start`/`observation_end`). Tiingo then issues **one HTTP request per ticker per day**. `TiingoProvider.fetch_date_range` and `FredProvider.fetch_date_range` exist and are used only by `scripts/backfill_5year.py`.

| Path | HTTP shape | Use when |
|------|------------|----------|
| `atlas run` / `atlas backfill` | One request per ticker (or FRED series) **per weekday** | Nightly / short date ranges, or FRED history (correct `obs_date`) |
| `python3 scripts/backfill_5year.py` | One request per ticker / series for the **entire ~5-year window** | Multi-year OHLCV only; FRED rows are currently dropped (looks for `date`) |

Example cost: 374 tagged tickers × ~1,260 weekdays ≈ **470k** Tiingo calls via CLI backfill, vs **374** range calls in the helper. Config `rate_limit_per_hour: 500` is **not enforced** in code (no sleep/token bucket). HTTP 429 raises `RateLimitError` and marks that ticker failed; tenacity retries only `TimeoutException` / `NetworkError` (3 attempts, exponential 2–30s). YAML `pipeline.retry.backoff_seconds` is unused. The helper sleeps 0.15s (Tiingo) / 0.25s (FRED) and holds the full result set in memory before a row-by-row upsert.

**FRED frequency:** a single-date fetch only persists observations whose `obs_date` equals that weekday. Monthly/quarterly series (GDP, payrolls, CPI) typically miss a nightly `atlas run`. Weekend publication dates are skipped because `skip_weekends=True`.
...
Roles are defined in code but not enforced on dashboard pages after login. There is no production fail-closed guard today: production still falls back to these defaults if `ATLAS_DASHBOARD_USERS` is missing. `get_users()` reads **only** that env var — `dashboard.auth.users_secret` (`atlas-dashboard-users`) is never loaded from Key Vault. The login page always prints the development credential hint. `dashboard.auth.method: azure_ad` is config-only; only simple username/password is implemented. Setting `dashboard.auth.enabled: false` skips login with no environment guard.

Sidebar pages: **Overview** (counts + latest 10 runs), **Price Data** (OHLCV chart/table + CSV download; ticker picker is the first 100 active instruments, max 10 selected), **Macro Data** (FRED category filter/chart), **Features** (V1 stub — not `FEATURE_CATALOG`), **Pipeline Runs** (filterable run history). `dashboard.default_lookback_days` and `max_export_rows` are unused (lookback is hard-coded to 30 days; exports are uncapped).
...
2. Register the instance in `setup_providers()` in `src/atlas/providers/registry.py`
3. Add provider config under `config/default.yaml` / env overlays
4. Add a persist branch in `PipelineOrchestrator._persist_results` — only the string names `"tiingo"` and `"fred"` write today; other registered providers fetch but are skipped with a warning

Provider `validate()` results are logged; **ERROR/WARNING does not block upsert**.
...
5. Note: `infrastructure/azure/modules/functionapp.bicep` hard-sets `ATLAS_ENV=production` on the Function App (app overlay), independent of `deploy.sh`'s Bicep environment name (`dev`/`staging`/`prod`)
...
| `ATLAS_DASHBOARD_USERS` | Dashboard auth | Recommended outside local use | JSON `{"user":"<sha256 hex>"}`; unset/invalid → default users. Key Vault `atlas-dashboard-users` is **not** read. |
...
| `atlas run` takes hours / hits Tiingo 429 | No `--tags` → `instruments is None` → full `/tiingo/daily` universe, one HTTP call per ticker; `rate_limit_per_hour` unused; 429 not retried | Seed CSV, `atlas instruments add-tag`, then `atlas run --tags <tag>`. CLI has no `--instruments` |
| Multi-year `atlas backfill` for Tiingo is extremely slow | CLI is per-weekday `fetch_data`, not `fetch_date_range` | Use `scripts/backfill_5year.py` for OHLCV history; keep `atlas backfill --providers fred` for macro until the helper's `date`/`obs_date` bug is fixed |
| Nightly FRED missing GDP/payrolls/CPI | `fetch_data` sets `observation_start=observation_end=target_date` | Expected for monthly/quarterly series; load history via a date-range path, not a single weekday run |
| Key Vault dashboard users ignored | `get_users()` only parses `ATLAS_DASHBOARD_USERS` | Set the env JSON; `dashboard.auth.users_secret` is unused |
| New provider fetches but DB stays empty | Persist is hard-coded to `"tiingo"` / `"fred"` | Add a persist branch in `_persist_results` |
| Price picker missing later tickers | Dashboard lists `ticker_options[:100]` | Query the DB or filter to the first 100 active instruments |
| Recalculated features keep old lineage | `FeatureRepository.upsert_batch` updates `value` only | Standalone V2 writes `fact_feature`; re-runs leave version/hash/transform/run_id stale |
...
| `pipeline.retry` / `max_parallel_workers` | `default.yaml` | Unused; HTTP retries are tenacity on timeout/network only |
| `providers.*.rate_limit_*` | `default.yaml` | Documented limits only; no token bucket. 429 is not retried |
| `dashboard.auth.users_secret` | `default.yaml` | Never loaded; auth is env-JSON or hard-coded defaults |
| `dashboard.default_lookback_days` / `max_export_rows` | `default.yaml` | Unused; Price page hard-codes 30 days and does not cap CSV rows |
...
A full `tests/` suite, `.pre-commit-config.yaml`, and Alembic migration tree are not present on `main` in this tree. `pytest` / pre-commit / Alembic remain declared dependencies or target workflow until those assets land. Schema creation is SQLAlchemy `create_all` via `atlas init-db` or automatically on first `PipelineOrchestrator.initialize()`. `get_settings()` is `@lru_cache`; change env overlays with `reload_settings()`. `get_database()` keeps the first connection string it sees.
