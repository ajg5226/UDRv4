...
def _coerce_row_date(row, *column_names):
    """Read and normalize the first available date-like column from a provider row."""
    for column_name in column_names:
        row_date = row.get(column_name)
        if row_date is None:
            continue
...
def build_macro_records(df, db_series_id: int, source_id: int) -> list[dict]:
    """Build FactMacro records from a FRED DataFrame."""
    records = []
    if not db_series_id or df.empty:
        return records

    for _, row in df.iterrows():
        if row.get("value") is not None:
            obs_date = _coerce_row_date(row, "obs_date", "date")
...
async def main():
    ...
    if failed_tickers:
        console.print(f"[yellow]Failed/empty tickers ({len(failed_tickers)}): {', '.join(failed_tickers[:15])}{'...' if len(failed_tickers) > 15 else ''}[/yellow]")
        ...
        for series_info in fred_series:
            ...
            try:
                ...
                if db_series_id and not df.empty:
                    macro_records.extend(build_macro_records(df, db_series_id, fred_source_id))
                elif df.empty:
    ...
    if failed_series:
        console.print(f"[yellow]Failed series ({len(failed_series)}): {', '.join(failed_series[:10])}{'...' if len(failed_series) > 10 else ''}[/yellow]")
    ...
    table.add_row("FRED Series Fetched", f"{len(fred_series) - len(failed_series)}/{len(fred_series)}")
    
    console.print(table)
    
    console.print("\nNext steps:")
    console.print("1. Calculate features: [cyan]PYTHONPATH=src python3 -c 'from atlas.features import ...'[/cyan]")
    console.print("2. Launch dashboard: [cyan]PYTHONPATH=src streamlit run src/atlas/dashboard/app.py[/cyan]")
