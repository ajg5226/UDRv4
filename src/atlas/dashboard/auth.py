...
def _parse_users(users_json: str) -> dict[str, str]:
    """Parse and validate dashboard user credentials JSON."""
    try:
...
def get_users() -> dict[str, str]:
    ...
    if users_json:
        ...
        if users:
            return users
        logger.warning("ATLAS_DASHBOARD_USERS is set but invalid JSON")

    # Try secrets manager (env -> Key Vault fallback)
    try:
        ...
        if users_secret:
            ...
            if users:
                return users
            logger.warning("Dashboard users secret is present but invalid JSON")
    except Exception as e:
        logger.warning("Unable to load dashboard users secret", error=str(e))

    # In production, fail closed instead of allowing default credentials.
    if settings.environment.lower() == "production":
        logger.error("Dashboard users are not configured in production")
        return {}
