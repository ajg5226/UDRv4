...
def _parse_users_json(users_json: str, source: str) -> dict[str, str]:
    """Parse and validate the configured dashboard users map."""
    try:
        users = json.loads(users_json)
    except json.JSONDecodeError as e:
        raise ConfigurationError(
            "Dashboard users configuration is not valid JSON",
    ...
    if invalid_users:
        raise ConfigurationError(
            "Dashboard users configuration contains invalid entries",
            details={"source": source, "users": invalid_users},
        )
...
def get_users() -> dict[str, str]:
    ...
    Outside development-like environments, credentials must be configured via
