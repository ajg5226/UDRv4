...
def _parse_users_json(users_json: str) -> dict[str, str]:
    """Parse dashboard users JSON payload into username->password_hash map."""
    try:
...
def get_users() -> dict[str, str]:
    ...
    # Try configured secret (Azure Key Vault in production)
    users_secret_value = get_secret(settings.dashboard.auth.users_secret)
    if users_secret_value:
        users = _parse_users_json(users_secret_value)
        if users:
            return users

    # Default users only for development-like environments (password: atlas123)
...
def show_login() -> None:
    ...
    settings = get_settings()
    if settings.environment.lower() in DEVELOPMENT_ENVIRONMENTS:
        # Development hint
        st.markdown("""
        ---
        ...
        *Set `ATLAS_DASHBOARD_USERS` environment variable or configure
        `atlas-dashboard-users` secret for production.*
        """)
    elif not get_users():
        st.error(
            "Dashboard users are not configured. Set ATLAS_DASHBOARD_USERS or "
            "configure the atlas-dashboard-users secret."
        )
