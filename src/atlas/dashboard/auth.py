...
def _is_development_environment() -> bool:
    """Return whether development-only dashboard defaults are allowed."""
    return get_settings().environment.lower() == "development"


def _parse_users_json(users_json: str) -> dict[str, str]:
    """Parse and validate the configured username-to-password-hash mapping."""
    try:
        users = json.loads(users_json)
    except json.JSONDecodeError as exc:
        raise AuthenticationError("Invalid ATLAS_DASHBOARD_USERS JSON") from exc

    if not isinstance(users, dict) or not users:
        raise AuthenticationError("Dashboard users must be a non-empty JSON object")

    if not all(
        isinstance(username, str) and isinstance(password_hash, str)
        for username, password_hash in users.items()
    ):
        raise AuthenticationError("Dashboard users must map usernames to password hashes")
...
def get_users() -> dict[str, str]:
    ...
    Loads from the environment or configured Key Vault secret. Development
    defaults are only allowed when ATLAS_ENV is development.
    ...
    if not _is_development_environment():
        ...
        raise AuthenticationError(
            "Dashboard users are not configured; set ATLAS_DASHBOARD_USERS "
            "or the configured Key Vault secret"
        )
