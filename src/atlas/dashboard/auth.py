...
def _is_development_environment() -> bool:
    """Return true only for environments where hardcoded dev users are allowed."""
    return get_settings().environment.lower() in DEVELOPMENT_ENVIRONMENTS


def _parse_users(users_json: str, source: str) -> dict[str, str]:
    """Parse username -> password-hash mappings from configured JSON."""
    try:
    ...
    if not isinstance(users, dict) or not users:
        raise ConfigurationError(f"Dashboard users in {source} must be a non-empty JSON object")
    ...
    if not all(
        ...
        raise ConfigurationError(
            f"Dashboard users in {source} must map usernames to password hashes"
        )
...
def get_users() -> dict[str, str]:
    ...
    In production, this loads from an environment variable or Key Vault.
    ...
    if not _is_development_environment():
        raise ConfigurationError(
            ...
            f"or Key Vault secret '{settings.dashboard.auth.users_secret}'"
        )
