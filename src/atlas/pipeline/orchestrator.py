...
class PipelineOrchestrator:
    ...
    def _get_providers(self, provider_names: list[str] | None) -> list:
        ...
        return [self._registry.get(name) for name in default_providers if self._registry.has(name)]

    async def _get_instruments(self, config: RunConfig) -> list[str] | None:
                ...
                try:
                    ...
                    if provider_name == "tiingo":
                        inserted, updated = await self._persist_ohlcv(
                            session, result, source.source_id, run_id, instrument_repo, ohlcv_repo
                        )
                    elif provider_name == "fred":
                        inserted, updated = await self._persist_macro(
                            session, result, source.source_id, run_id, macro_series_repo, macro_repo
                        )
                    else:
