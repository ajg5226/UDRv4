...
def _is_missing_value(value) -> bool:
    """Return True for values that should not overwrite existing data."""
    if value is None:
        return True
...
def _as_float_or_none(value) -> Optional[float]:
    """Convert numeric values while preserving legitimate zero values."""
    if value is None:
        return None
    return float(value)
        ...
        records = [
            {
                ...
                "open": _as_float_or_none(r.open),
                "high": _as_float_or_none(r.high),
                "low": _as_float_or_none(r.low),
                "close": _as_float_or_none(r.close),
                "volume": r.volume,
                "adj_open": _as_float_or_none(r.adj_open),
                "adj_high": _as_float_or_none(r.adj_high),
                "adj_low": _as_float_or_none(r.adj_low),
                "adj_close": _as_float_or_none(r.adj_close),
        ...
        records = [
            {
                ...
                "obs_date": r.obs_date,
                "value": _as_float_or_none(r.value),
            }
            for r in results.scalars()
        ]
        return pd.DataFrame(records)
