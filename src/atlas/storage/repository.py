...
def _is_missing_value(value: object) -> bool:
    """Return True for scalar values that should be treated as missing."""
    if value is None:
        return True
    try:
...
def _to_float_or_none(value: object) -> Optional[float]:
    """Convert database numeric values to float without treating zero as missing."""
    ...
    def upsert_batch(
        ...
        for record in records:
            ...
            if existing:
                ...
                # Insert
                self.session.add(FactOHLCV(**record))
                inserted += 1
