...
def test_dashboard_main_rejects_disabled_auth_outside_development(monkeypatch):
    ...
    monkeypatch.setattr(dashboard_app.st, "error", errors.append)

    dashboard_app.main()

    assert errors == ["Dashboard authentication must be enabled outside development."]
